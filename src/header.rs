...
use image::{ColorType, DynamicImage, EncodableLayout, GenericImage, GenericImageView, Pixel, Rgba};
...
impl TryInto<HeaderRaw> for VersionedHeader {
    ...
    fn try_into(self) -> Result<HeaderRaw, Self::Error> {
        ...
        Ok(
            HeaderRaw { magic: 0x42, header_len: data.len() as u16, data, crc }
        )
    }
}
...
impl TryFrom<HeaderRaw> for VersionedHeader {
    ...
    fn try_from(value: HeaderRaw) -> Result<Self, Self::Error> {
        ...
        if crc != value.crc {
            return Err(format!("Checksum Mismatch. Expected {:#01x}, but found {:#01x}", crc, value.crc))
        }
...
fn calculate_bit_mask(bits_needed_per_pixel: u8, color_type: ColorType) -> u64 {
    ...
    let mut data_bits_per_channel = vec![bit_count_on_all_channels; color_type.channel_count() as usize];
...
pub(crate) fn generate_v1_header(pixel_count: u64, data_len_bytes: u64, color_type: ColorType) -> Result<VersionedHeader, String> {
    ...
    let offset = v1_header_len + thread_rng().gen_range(0..(available_pixels - pixels_needed_to_store_message));
...
mod tests {
    ...
    fn calculate_bit_mask_rgb8() {
        ...
        assert_eq!(format!("{:#01x}",response) ,  format!("{:#01x}", 0x0F_0F_0F_00__00_00_00_00u64))
    }
    ...
    fn calculate_bit_mask_rgba8() {
        ...
        assert_eq!(format!("{:#01x}",response) ,  format!("{:#01x}", 0x0F_0F_0F_0F__00_00_00_00u64))
    }

    #[test]
    fn calculate_partial_bit_mask_rgba8() {
        let response = calculate_bit_mask(5, ColorType::Rgba8);
        assert_eq!(format!("{:#01x}",response) ,  format!("{:#01x}", 0x03_01_01_01__00_00_00_00u64))
    }
    ...
    fn vectorize_and_unvectorize_bit_mask() {
        ...
        assert_eq!(format!("{:#01x}",bitmask) ,  format!("{:#01x}", back_transform))
    }
